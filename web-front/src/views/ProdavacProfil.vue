<template>
  <div id="prodavacProfil" class="container mt-5">
    <h1 class="text-center">Profil Prodavca</h1>
    <div v-if="prodavac" class="card p-3 mb-4">
      ...
      <h2 class="text-center mb-3">Proizvodi koje prodaje</h2>
      <ul>
      ...
      <button @click="oceniProdavca" class="btn btn-outline-primary btn-sm">Oceni Prodavca</button>
      <button @click="prijaviProdavca" class="btn btn-outline-danger btn-sm">Prijavi Prodavca</button>
    </div>
...
export default {
  ...
  methods: {
    ucitajProdavca() {
      ...
      axios
        .get(`http://localhost:8081/prodavac/${id}`, {
          headers: {
    ...
    ucitajProdajeProizvode() {
      const id = this.$route.query.id;
      axios
        .get(`http://localhost:8081/prodavac/${id}/proizvodi`, {
          headers: {
            Accept: "application/json"
          },
          withCredentials: true
    ...
    oceniProdavca() {
      const id = this.$route.query.id;
      this.$router.push({ path: '/oceniProdavca', query: { id } });
    },
    prijaviProdavca() {
      const id = this.$route.query.id;
      this.$router.push({ path: '/prijaviProdavca', query: { id } });
    }
  }
};
</script>
