<template>
    <div class="container mt-5">
      <div class="card p-3 mb-4">
        <h1 class="text-center">Oceni Prodavca</h1>
        <div v-if="prodavac">
          <p><strong>Ime prodavca:</strong> {{ prodavac.ime }} {{ prodavac.prezime }}</p>
  ...
  export default {
    ...
    methods: {
      ...
      posaljiOcenu() {
        ...
        const id = this.$route.query.id;
        axios
          .post(`http://localhost:8081/api/oceni-prodavca/${id}`, recenzijaDto, { withCredentials: true })
          .then(() => {
            alert("Ocena je uspešno poslata!");
            this.$router.push({ path: '/prodavacProfil', query: { id } });
          })
          .catch((err) => {
            console.error("Error:", err);
